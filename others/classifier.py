...
class CLASSIFIER:
    ...
    def val_gzsl(self, test_X, test_label, target_classes): 
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                ...
                inputX = Variable(test_X[start:end], reqires_grad=True)
    ...
    def val(self, test_X, test_label, target_classes): 
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                inputX = Variable(test_X[start:end].cuda(), requires_grad=True)
            else:
                inputX = Variable(test_X[start:end], requires_grad=True)
            output = self.model(inputX) 
            _, predicted_label[start:end] = torch.max(output.data, 1)
            start = end
